...
void NormalMapShaderClass::EnableShaderForFrame(const XMMATRIX& vp_matrix,
...
void NormalMapShaderClass::ProcessRenderQueue(ID3D11DeviceContext* device_context)
{
	...
	{
		throw GAME_EXCEPTION(L"NormalMapShaderClass::ProcessRenderQueue() called without enabling the shader for the frame.");
	}
	...
	for (auto& [model, params] : render_queue_)
	{
		...
		for (const auto& param : params)
		{
			...
			SetShaderParameters(device_context, param.world_matrix, render_constant_.vp_matrix,
				...
				render_constant_.light_direction, render_constant_.diffuse_color, render_constant_.camera_pos,
